...
def train(epochs, print_every=1, learning_rate=0.1, hidden_size=6, num_of_layers=1, dropout=0):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print('Using {} for training'.format(device))

    model = net.LSTM(hidden_size=hidden_size, num_of_layers=num_of_layers, dropout=dropout)
